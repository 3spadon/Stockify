...
function check_user(){
  ...
  if (!empty($_SESSION['username']) && !empty($_SESSION['password'])){ //Si les session pseudo et mot de passe ont été initialisées
      ...
      //Si la requ�te ne renvoie rien c'est qu'elle est malformée, potentiellement une injection SQL, on alerte un potentiel attaquant
    ...
    //On invite la personne non connectée à le faire en le redirigeant. La redirection a pour option 'no_user_logged' afin d'afficher le bon message d'erreur sur la page de connexion
  header('Location: index.php?err=no_user_logged');
  }
...
function getLastConnection(){
  //On démarre la session afin d'extraire une variable SESSION à savoir $_SESSION['dateDerniereConnexion']
  session_start();
